...
{
    public abstract class DataTableRateLimiter : IRateLimiter
    {
        private static readonly DateTime Epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        private static readonly long? RateLimitOverride;

        private readonly ICache<RateLimitDataTableEntity> rateLimiterCache;
        private readonly ICache<RateLimitDataTableEntity> rateLimiterCacheCosmosDb;
        protected ITelemetryClient TelemetryClient { get; private set; }
        private readonly bool expectRateLimitingHeaders;
        // TODO (myles-mcleroy): Remove dual writing when Cosmos DB migration is complete.
        ...
        private RateLimitDataTableEntity cachedResult;
        private DateTime cacheDateUtc;

        static DataTableRateLimiter()
        {
        ...
        public DataTableRateLimiter(string organizationId,
                           string organizationName,
                           ICache<RateLimitDataTableEntity> rateLimiterCache,
                           ITelemetryClient telemetryClient,
                           bool expectRateLimitingHeaders,
                           ICache<RateLimitDataTableEntity> rateLimiterCacheCosmosDb = null,
        ...
        public async Task UpdateRetryAfterAsync(string identity, string requestUrl, HttpResponseMessage response)
        {
            ...
            var existingRecord = await this.rateLimiterCache.RetrieveAsync(new RateLimitDataTableEntity(identity, this.OrganizationId, this.OrganizationName)).ConfigureAwait(false);
            ...
            this.cachedResult = new RateLimitDataTableEntity(identity, this.OrganizationId, this.OrganizationName, rateLimitLimit, rateLimitRemaining, rateLimitResetDate, retryAfterDate);
        ...
        public async Task UpdateStatsAsync(string identity, string requestUrl, HttpResponseMessage response)
        {
            ...
            var rateLimitTableEntity = new RateLimitDataTableEntity(identity, this.OrganizationId, this.OrganizationName, rateLimitLimit, rateLimitRemaining, rateLimitResetDate, retryAfterDate);
        ...
        public async Task WaitIfNeededAsync(IAuthentication authentication)
        {
            ...
            {
                this.cachedResult = await this.rateLimiterCache.RetrieveAsync(new RateLimitDataTableEntity(authentication.Identity, this.OrganizationId, this.OrganizationName)).ConfigureAwait(false);
                this.cacheDateUtc = DateTime.UtcNow;
            }
        ...
        protected abstract Task WaitIfNeededAsync(IAuthentication authentication, RateLimitDataTableEntity tableEntity);
    }
}
